#-------------------------------------------------------------------
# This module builds Grafana infrastructure
# - EC2 instance with EBS root volume
# - Route 53 DNS record
...
variable cluster_component { default = "grafana" }
...
resource "aws_route53_record" "grafana_vip" {
  count = "${var.instance_count}"
  zone_id = "${var.dns_zone_id}"
...
# CloudWatch Alarms
#-------------------------------------------------------------------
resource "aws_cloudwatch_metric_alarm" "grafana_ec2_autorecover" {
  ...
  dimensions {
      InstanceId = "${element(aws_instance.grafana.*.id, count.index)}"
  }
}

output "vip" {
  value = ["${element(aws_route53_record.grafana_vip.*.fqdn, 0)}"]
}
output "hostnames" {
