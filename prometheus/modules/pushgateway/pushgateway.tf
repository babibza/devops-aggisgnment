#-------------------------------------------------------------------
# This module builds Prometheus PushGateway infrastructure
...
variable cluster_component { default = "pushgateway" }
...
# Route53 DNS Records
#-------------------------------------------------------------------
resource "aws_route53_record" "pushgateway" {
  count = "${var.instance_count}"
  zone_id = "${var.dns_zone_id}"
  name = "${module.hostnames.hostnames[count.index]}"
  type = "A"
  ttl = "300"
  records = ["${element(aws_instance.pushgateway.*.private_ip, count.index)}"]
}

resource "aws_route53_record" "pushgateway_vip" {
  count = "${var.instance_count}"
  zone_id = "${var.dns_zone_id}"
...
# CloudWatch Alarms
#-------------------------------------------------------------------
resource "aws_cloudwatch_metric_alarm" "pushgateway_ec2_autorecover" {
  alarm_name          = "${module.hostnames.hostnames[count.index]}-ec2-autorecover"
  count               = "${var.instance_count}"
  namespace           = "AWS/EC2"
  evaluation_periods  = "2"
  period              = "60"
  alarm_description   = "This metric alarm auto recovers Prometheus PushGateway EC2 instances"
  alarm_actions       = [
  ...
  dimensions {
      InstanceId = "${element(aws_instance.pushgateway.*.id, count.index)}"
  }
}

output "vip" {
  value = ["${element(aws_route53_record.pushgateway_vip.*.fqdn, 0)}"]
}
output "hostnames" {
  value = ["${aws_route53_record.pushgateway.*.fqdn}"]
}
output "private_ips" {
  value = ["${aws_instance.pushgateway.*.private_ip}"]
}
output "instances" {
  value = ["${aws_instance.pushgateway.*.id}"]
}
output "names" {
  value = ["${aws_instance.pushgateway.*.tags.Name}"]
}
